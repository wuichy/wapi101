...
module.exports = function templateTagsRoutes(db) {
  ...
  router.post('/', (req, res) => {
    ...
    const dupe = db.prepare('SELECT id FROM template_tags WHERE tenant_id = ? AND name = ? COLLATE NOCASE').get(req.tenantId, name);
    if (dupe) return res.status(409).json({ error: 'Ya existe una etiqueta con ese nombre' });
    try {
      const r = db.prepare('INSERT INTO template_tags (tenant_id, name, color) VALUES (?, ?, ?)').run(req.tenantId, name, color);
  ...
  router.put('/:id', (req, res) => {
    ...
    const color = String(req.body?.color || '#94a3b8').trim();
    if (!name) return res.status(400).json({ error: 'Nombre requerido' });
    const existing = db.prepare('SELECT id FROM template_tags WHERE id = ? AND tenant_id = ?').get(id, req.tenantId);
    if (!existing) return res.status(404).json({ error: 'Etiqueta no encontrada' });
    const dupe = db.prepare('SELECT id FROM template_tags WHERE tenant_id = ? AND name = ? COLLATE NOCASE AND id <> ?').get(req.tenantId, name, id);
    if (dupe) return res.status(409).json({ error: 'Ya existe una etiqueta con ese nombre' });
    try {
      db.prepare('UPDATE template_tags SET name = ?, color = ? WHERE id = ? AND tenant_id = ?').run(name, color, id, req.tenantId);
  ...
  router.delete('/:id', (req, res) => {
    const id = Number(req.params.id);
    db.prepare('DELETE FROM template_tags WHERE id = ? AND tenant_id = ?').run(id, req.tenantId);
    res.json({ ok: true });
  });
