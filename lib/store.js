...
function addMessageToChat(chatId, message) {
  return upsertChat(chatId, (chat) => {
    ...
    // Si es un mensaje saliente confirmado por webhook (ID real, no local),
    // buscar si ya existe un mensaje local con el mismo texto para reemplazarlo
    // en vez de agregar un duplicado.
    ...
    if (message.direction === 'outgoing' && !messageId.startsWith('local-')) {
      ...
      if (localIdx >= 0) {
        // Reemplazar el marcador local con el mensaje confirmado
