...
async function updateContactIfNeeded({ existing, contact, orderNumber }) {
  ...
  // ── Actualizar nombre/apellido si Woo trae datos y difieren del contacto ──
  // Woo billing es la fuente de verdad: el cliente acaba de llenar el form.
  // Solo actualizamos si trae valor no vacío (nunca borra datos existentes).
  const incomingFull = (contact.fullName || [contact.firstName, contact.lastName].filter(Boolean).join(' ')).trim();
  if (incomingFull && incomingFull !== (existing.name || '').trim()) {
    patch.name = incomingFull;
  }
  if (contact.firstName && contact.firstName !== (existing.first_name || '')) {
    patch.first_name = contact.firstName;
  }
  if (contact.lastName && contact.lastName !== (existing.last_name || '')) {
    patch.last_name = contact.lastName;
  }
...
async function updateLeadOnNewOrder({ lead, order, contactName }) {
  const patch = {};

  // ── Renombrar el lead a "{nombreCompleto} {dd/mm/aa}" ──
  // Convención de Reelance Hub para que el lead refleje el nombre real del
  // cliente (no el default "Lead #xxxxxx" de Kommo) + fecha del último pedido.
  const desiredName = `${contactName || 'Cliente'} ${order.orderDateFormatted}`.trim();
  if (desiredName && desiredName !== (lead.name || '').trim()) {
...
async function syncOrder(order) {
  ...
  let leadRecord;
  if (openLead) {
    leadRecord = await updateLeadOnNewOrder({
      lead: openLead,
      order,
